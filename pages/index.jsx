      ...
      <div style={{background:BG2,border:`1px solid ${top3.length?'rgba(255,0,144,0.2)':BDR}`,padding:'16px 18px',display:'flex',flexDirection:'column',gap:0,minWidth:0}}>
        ...
        {top3.length === 0 ? (
          ...
          top3.map((e, i) => {
            ...
            return (
              <div key={e.id}
                style={{display:'flex',alignItems:'center',gap:10,padding:'8px 0',borderBottom:i<top3.length-1?`1px solid ${BDR}`:'none'}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontFamily:SANS,fontWeight:700,fontSize:13,color:TXT,display:'flex',alignItems:'center',gap:6}}>
                    <span style={{whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{e.player}</span>
                    {org?.country && <span style={{display:'inline-flex',width:16,height:12,overflow:'hidden',flexShrink:0}}>{countryFlag(org.country)}</span>}
                  </div>
