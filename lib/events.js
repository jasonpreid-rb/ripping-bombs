...
// Returns 'upcoming' | 'active' | 'ended' based on startAt/endAt vs now.
// startAt/endAt are required on create, but this falls back to 'active'
// defensively rather than hiding an event if either is ever missing.
export function getEventStatus(event) {
  if (!event?.startAt || !event?.endAt) return 'active';
  const now = Date.now();
...
// Same as getEventStatus, but looks the event up by id — used at submit
// time (doSubmit in _app.jsx) where only form.eventId is on hand, not the
// full event object from getEventBySlug. Returns null if the event can't
// be found, so callers can choose to fail open or closed as appropriate.
export async function getEventStatusById(eventId) {
  const { data, error } = await supabase.from('events').select('startAt, endAt').eq('id', eventId).maybeSingle();
  if (error || !data) return null;
  return getEventStatus(data);
}

// Like getEventStatusById, but also returns the venue's per-player attempt
// cap for the event (null = unlimited) — used at submit time to enforce it
// alongside the window check.
export async function getEventSubmissionInfo(eventId) {
  const { data, error } = await supabase.from('events').select('startAt, endAt, maxEntriesPerPlayer').eq('id', eventId).maybeSingle();
  if (error || !data) return null;
  return { status: getEventStatus(data), maxEntriesPerPlayer: data.maxEntriesPerPlayer };
}
