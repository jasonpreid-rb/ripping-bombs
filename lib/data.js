...
export const db = {
  ...
  // Admin-authenticated update — bypasses RLS via the server-side service-role
  // client. Use this for admin-panel edits (status changes, editing club
  // details); keep the plain updateOrg() above for public self-service writes
  // like the consent flow, which run under RLS with the anon/publishable key.
  async adminUpdateOrg(id, fields) {
    try {
      const password = typeof window !== 'undefined' ? localStorage.getItem('rb_admin_pw') : null;
      const res = await fetch('/api/admin/update', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ password, id, fields }),
      });
      const json = await res.json();
      if (!json.ok) { console.error('ADMIN UPDATE ORG ERROR:', json.error); return false; }
      return true;
    } catch (e) {
      console.error('ADMIN UPDATE ORG EXCEPTION:', e);
      return false;
    }
  },
  ...
  async deleteOrg(id) {
    try {
      ...
      const res = await fetch('/api/admin/delete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ password, type: 'org', id }),
      });
      const json = await res.json();
      if (!json.ok) { console.error('DELETE ORG ERROR:', json.error); return false; }
      return true;
    } catch (e) {
  ...
  async deleteEntry(id) {
    try {
      ...
      const res = await fetch('/api/admin/delete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ password, type: 'entry', id }),
      });
      const json = await res.json();
      if (!json.ok) { console.error('DELETE ENTRY ERROR:', json.error); return false; }
      return true;
    } catch (e) {
...
// A row is sample/demo data if it came from either seeding mechanism used
// during development: the newer `demo_`-prefixed rows, or the older
// SEED_ORGS/SEED_ENTRIES rows below (ids like 'o6', 'e01'). Real ids are
// always Date.now().toString() — a long numeric timestamp — so this can't
// collide with genuine club or entry data.
export function isSampleId(id) {
  return typeof id === 'string' && (id.startsWith('demo_') || /^[oe]\d+$/.test(id));
}
...
// Loads live data from Supabase. No longer auto-seeds or falls back to
// fake data — an empty or errored read now surfaces as an empty state in
// the UI rather than silently substituting SEED_ORGS/SEED_ENTRIES. (This
// used to seed the clubs table with 16 fictional venues if it was ever
// empty, which meant a rare edge case could silently repopulate production
// with fake data — removed along with the fallback for the same reason.)

export async function initData() {
  try {
    const supabase = await getSupabase();

    // Fetch both tables in parallel; exclude photo (large base64) from list query
    const [{ data: orgs, error: orgsErr }, { data: entries, error: entriesErr }] = await Promise.all([
      supabase.from('clubs').select('*'),
      // player_email, venueId, and facility are intentionally excluded here —
      // venueId/facility aren't needed by the shared public entries state
      // (the TV display fetches them directly via lib/venueDisplayData.js),
      // and player_email must never enter the client-side entries array.
      supabase.from('entries').select('id,orgId,player,dist,club,hcp,age,date,tournament,gender,is_simulator'),
    ]);

    if (orgsErr) console.error('INIT DATA — clubs read error:', orgsErr);
    if (entriesErr) console.error('INIT DATA — entries read error:', entriesErr);

    return {
      orgs:    orgs    || [],
      entries: entries || [],
      error:   Boolean(orgsErr || entriesErr),
    };

  } catch (e) {
    console.error('INIT DATA ERROR:', e);
    return { orgs: [], entries: [], error: true };
  }
}
