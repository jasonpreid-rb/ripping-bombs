...
// NOTE: courseName values below are fictional on purpose (see
// docs/01-rename-club-names.sql for the matching live-data migration) —
// these previously used real, trademarked club names, which is a legal
// risk when paired with fabricated "verified" results. Keep this file and
// the live Supabase data in sync going forward: initData() only seeds
// these rows into an empty table, so editing this array alone does NOT
// change what's already live.
export const SEED_ORGS = [
  { id:'o1',  fullName:'James Hargreaves', position:'Club Secretary',      courseName:'Old Saltmarsh Golf Club',       location:'Southport, England',     country:'GB', email:'james@oldsaltmarsh.example',  pw:'demo', logo:'', status:'approved', badge:'platinum', accountType:'club', simulator:'' },
  { id:'o2',  fullName:"Caitlin O'Brien",  position:'Tournament Director',  courseName:'Dunbeacon Links',               location:'Lahinch, Ireland',        country:'IE', email:'caitlin@dunbeaconlinks.example',       pw:'demo', logo:'', status:'approved', badge:'tour',     accountType:'club', simulator:'' },
  { id:'o3',  fullName:'Magnus Lindqvist', position:'Head Professional',    courseName:'Öresund Golf & Country Club',   location:'Malmo, Sweden',           country:'SE', email:'magnus@oresundgcc.example',      pw:'demo', logo:'', status:'approved', badge:'platinum', accountType:'club', simulator:'' },
  { id:'o4',  fullName:'Hiroshi Tanaka',   position:'General Manager',      courseName:'Rokko Hills Golf Club',         location:'Kobe, Japan',             country:'JP', email:'hiroshi@rokkohills.example',        pw:'demo', logo:'', status:'approved', badge:'tour',     accountType:'club', simulator:'' },
  { id:'o5',  fullName:'Priya Nair',       position:'Events Coordinator',   courseName:'Hooghly Golf Club',             location:'Kolkata, India',          country:'IN', email:'priya@hooghlygolf.example',            pw:'demo', logo:'', status:'approved', badge:'amateur',  accountType:'club', simulator:'' },
  { id:'o6',  fullName:'Dylan Schwartz',   position:'Club Manager',         courseName:'Blackwood Ridge Golf Club',     location:'New York, USA',           country:'US', email:'dylan@blackwoodridge.example',       pw:'demo', logo:'', status:'approved', badge:'platinum', accountType:'club', simulator:'' },
  { id:'o7',  fullName:'Amara Diallo',     position:'Tournament Organiser', courseName:'Kruger Vista Country Club',     location:'Malelane, South Africa',  country:'ZA', email:'amara@krugervista.example', pw:'demo', logo:'', status:'approved', badge:'tour',     accountType:'club', simulator:'' },
  { id:'o8',  fullName:'Sofia Reyes',      position:'Head Pro',             courseName:'Sierra Volcán Golf Club',       location:'Mexico City, Mexico',     country:'MX', email:'sofia@sierravolcan.example',     pw:'demo', logo:'', status:'approved', badge:'amateur',  accountType:'club', simulator:'' },
  { id:'o9',  fullName:'Hans Brauer',      position:'Head Professional',    courseName:'Golf Club Isartal',             location:'Bavaria, Germany',        country:'DE', email:'hans@gcisartal.example',             pw:'demo', logo:'', status:'approved', badge:'tour',     accountType:'club', simulator:'' },
  { id:'o10', fullName:'Will Cartwright',  position:'General Manager',      courseName:'Craigmoor Links',               location:'Carnoustie, Scotland',    country:'GB', email:'will@craigmoorlinks.example',    pw:'demo', logo:'', status:'approved', badge:'platinum', accountType:'club', simulator:'' },
  { id:'o11', fullName:'Nadia Okonkwo',    position:'Club Director',        courseName:'Lekki Bay Golf Club',           location:'Lagos, Nigeria',          country:'NG', email:'nadia@lekkibaygolf.example',           pw:'demo', logo:'', status:'approved', badge:'amateur',  accountType:'club', simulator:'' },
  { id:'o12', fullName:'Pedro Almeida',    position:'Tournament Director',  courseName:'Vale Dourado Golf Club',        location:'Algarve, Portugal',       country:'PT', email:'pedro@valedourado.example',             pw:'demo', logo:'', status:'approved', badge:'tour',     accountType:'club', simulator:'' },
  { id:'o13', fullName:'Chen Wei',         position:'Head Professional',    courseName:'Nanshan Ridge Golf Club',       location:'Shenzhen, China',         country:'CN', email:'chen@nanshanridge.example',     pw:'demo', logo:'', status:'approved', badge:'platinum', accountType:'club', simulator:'' },
  { id:'o14', fullName:'Aiden Murphy',     position:'Events Manager',       courseName:'Liffey Vale Golf Resort',       location:'Straffan, Ireland',       country:'IE', email:'aiden@liffeyvale.example',           pw:'demo', logo:'', status:'approved', badge:'tour',     accountType:'club', simulator:'' },
  { id:'o15', fullName:'Fatima Al-Rashid', position:'Club Secretary',       courseName:'Al Reem Dunes Golf Club',       location:'Abu Dhabi, UAE',          country:'AE', email:'fatima@alreemdunes.example',           pw:'demo', logo:'', status:'pending',  badge:null,       accountType:'club', simulator:'' },
  { id:'o16', fullName:'Marco Rossi',      position:'General Manager',      courseName:'Lombardia Fairways Golf Club',  location:'Milan, Italy',            country:'IT', email:'marco@lombardiafairways.example',        pw:'demo', logo:'', status:'pending',  badge:null,       accountType:'club', simulator:'' },
];
...
// True if this id belongs to the seed/demo data rather than a real
// registered account or submission — used by pages like clubs/[slug].jsx
// to label or exclude sample entries. Reconstructed from the id
// convention used elsewhere in this file: seed rows use short ids like
// 'o1'/'e01' (see SEED_ORGS/SEED_ENTRIES below), while real accounts and
// submissions get a Date.now().toString() id (see doRegister/doSubmit in
// _app.jsx). If this doesn't match the original behavior, let me know
// what it should check instead.
// Broadened beyond the original SEED_ORGS/SEED_ENTRIES check to also catch
// demo/sample rows seeded outside this file (admin tool, manual imports):
// demo_sim_*, sim_demo_*, and simdemo<digits>_* id prefixes. Add any new
// demo-seeding convention's prefix here — and keep the matching check in
// scripts/generate-sitemap.cjs in sync, since that script duplicates this
// logic standalone outside the Next.js module graph.
export function isSampleId(id) {
  if (!id) return false;
  if (SEED_ORGS.some((o) => o.id === id) || SEED_ENTRIES.some((e) => e.id === id)) return true;
  return /^(demo_|sim_demo_|simdemo\d+_)/i.test(id);
}
