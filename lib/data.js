...
export async function initData() {
  try {
    ...
    const [{ data: orgs }, { data: entries }] = await Promise.all([
      ...
      // player_email is intentionally excluded — it's insert-only and must never
      // enter the client-side entries array, since that state feeds the public leaderboard.
      supabase.from('entries').select('id,orgId,player,dist,club,hcp,age,date,tournament,gender,is_simulator,venue_id,facility_name'),
    ]);
